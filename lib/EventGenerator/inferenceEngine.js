...
*/ 
InferenceEngine.prototype.getRulesToFire = function(VarOptsSet){
	if(VarOptsSet.length != this.varOptCnts.length) throw "Wrong number of variables supplied";
	 
	// its faster to copy the first bitArray than set a bitArray to all ones
	var matches = this.GetRulesToFireInVariable(0,VarOptsSet[0]);
	
	for(var i = 1;i<VarOptsSet.length;i++)
	{
		matches.and( this.GetRulesToFireInVariable(i,VarOptsSet[i]) );
	}
	return matches;
}
